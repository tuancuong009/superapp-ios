...
struct EditCarView: View {
    ...
    private func updateUI(){
        addCarModel.make = carModel.make
        addCarModel.year = carModel.year
        addCarModel.model = carModel.model
        addCarModel.rentDay = carModel.rent
        addCarModel.rentweek = carModel.rentw
        addCarModel.rentMonth = carModel.rentm
        addCarModel.location = carModel.address
        addCarModel.city = carModel.city
        addCarModel.zipCode = carModel.zip
        addCarModel.description = carModel.desc
        stateID = carModel.state
        countryID = carModel.country
        addCarModel.id = carModel.id
        isRent = carModel.type
            ...
            APIKarkonexHelper.shared.getCountries { success, dict in
                if let dict = dict{
                    ...
                    for dictionary in dict {
                        if let id = dictionary.object(forKey: "id") as? String, id == countryID{
                            addCarModel.country = dictionary
                            break
                        }
                    }
                }
        ...
        if addCarModel.states.isEmpty{
            APIKarkonexHelper .shared.getStates(countryId: self.countryID) { success, dict in
                if let dict = dict{
                    isApiState = false
                    addCarModel.states = dict
                    for dictionary in dict {
                        if let id = dictionary.object(forKey: "id") as? String, id == stateID{
                            stateName = dictionary.object(forKey: "name") as? String ?? ""
                            addCarModel.state = dictionary
                            break
                        }
                    }
                }
    ...
    private func callApiAddCar(){
        if addCarModel.make.trimText().isEmpty{
            ...
            amountIsFocused = false
            isLoading = true
            let param = ["id": addCarModel.id, "uid": Auth.shared.getUserId(), "make": addCarModel.make , "modal": addCarModel.model, "year": addCarModel.year, "rent": addCarModel.rentDay, "rentw": addCarModel.rentweek, "rentm": addCarModel.rentMonth, "address": addCarModel.location, "discription": addCarModel.description, "city": addCarModel.city, "zip": addCarModel.zipCode, "state": stateID, "country": countryID, "type": isRent ? "0" : "1"]
            APIKarkonexHelper.shared.editCar(param, photo1, photo2, photo3, photo4) { success, errer in
                isLoading = false
                if success!{
                    self.editSuccess()
