...
import Alamofire
import SDWebImage
class EditProfileMatcheronViewController: BaseMatcheronViewController {
    ...
    @IBAction func doParingOpen(_ sender: Any) {
        if let dictUser = dictUser, let pairing = dictUser.object(forKey: "pairing") as? String, !pairing.isEmpty{
            return
        }
...
//            if !cancel{
//                let vc = ReligionPopUpViewController.init()
//                vc.religion = value ?? ""
...
//        }
    ...
    @IBAction func doCountry(_ sender: Any) {
        ...
        if let dictUser = dictUser, let country = dictUser.object(forKey: "country") as? String, !country.isEmpty{
            return
        }
        var values = [String]()
    ...
    func callApi(){
        ...
        param["phone"] = phone
        param["email"] = email
        param["id"] = UserDefaults.standard.value(forKey: USER_ID) as? String ?? ""
        if !RegionId.isEmpty{
        ...
        if imageEdit1 == nil && imageEdit2 == nil && imageEdit3 == nil{
            ...
            APIHelper.shared.profileUpdate(param) { success, erro in
                ...
                if success!{
                    if let err = erro{
                        self.showAlertMessageComback(err, complete: { success in
                            self.navigationController?.popViewController(animated: true)
                        })
                    }
                    else{
            ...
            SDImageCache.shared.clearMemory()
            SDImageCache.shared.clearDisk()
            APIHelper.shared.updatePhotos(image1: imageEdit1, image2: imageEdit2, image3: imageEdit3) { success, errer in
                APIHelper.shared.profileUpdate(param) { success, erro in
                    self.hideBusy()
                    if success!{
                        if let err = erro{
...
extension EditProfileMatcheronViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate
{
...
extension EditProfileMatcheronViewController{
    private func updateUI(){
        ...
        if let dictUser = dictUser{
            ...
            if !txfAge.text!.trimText().isEmpty{
                viewAge.backgroundColor = .init(hex: "939393").withAlphaComponent(0.5)
                txfAge.isEnabled = false
            }
            txfHeight.text = dictUser.object(forKey: "height") as? String
            
            txfWeight.text = dictUser.object(forKey: "weight") as? String
            if let phoneStatus =  dictUser.object(forKey: "phonestatus") as? String{
                btnPhoneVisible.isSelected = phoneStatus == "1" ? true : false
                btnPhoneHidden.isSelected = phoneStatus == "1" ? false : true
            }
            else if let phoneStatus =  dictUser.object(forKey: "phonestatus") as? Int{
                btnPhoneVisible.isSelected = phoneStatus == 1 ? true : false
                btnPhoneHidden.isSelected = phoneStatus == 1 ? false : true
            }
            ...
            if !txfCountry.text!.trimText().isEmpty{
                viewCurrentCountry.backgroundColor = .init(hex: "939393").withAlphaComponent(0.5)
            }
...
extension  EditProfileMatcheronViewController: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout{
...
extension EditProfileMatcheronViewController: UITextViewDelegate{
