...
class PremierCircleShowcaseController: BaseViewController {
    ...
    private let dataSource = ["Personal Account", "Business Account"]
...
extension PremierCircleShowcaseController {
    ...
    private func setupDropdown() {
        ...
        dropDown.selectionAction = {[weak self] (index: Int, item: String) in
            ...
            self.searchUser(self.currentSearchTerm)
        }
    }
    ...
    @objc private func refresh() {
        ...
        searchUser(self.currentSearchTerm)
    }
}
...
extension PremierCircleShowcaseController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        var height: CGFloat = width*0.86 + 70
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if user.id == AppSettings.shared.userLogin?.userId {
            navigationController?.setViewControllers([MyProfileViewController.instantiate()], animated: true)
        } else {
...
extension PremierCircleShowcaseController: UITextFieldDelegate {
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        guard let term = searchTextField.text?.trimmed, !term.isEmpty else { return false }
