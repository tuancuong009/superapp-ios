...
import AVKit
import AVFoundation
class SendMessageSuperVC: BaseMatcheronViewController {
    ...
    @IBAction func doSend(_ sender: Any) {
        ...
        let userID = UserDefaults.standard.value(forKey: USER_ID_SUPER_APP) as? String ?? ""
        
        let param = ["sid": userID, "rid": profileID, "message": message]
        ManageAPI.shared.addMSGSuperApp(param) { success, erro in
            if success!{
    ...
    func getMessages(){
        ManageAPI.shared.getMessageSuperAppsByUser(profileID) { success, dict in
    ...
    private func addMessageIMG(_ image: UIImage){
        ...
        ManageAPI.shared.addMessageImageSuperApps(image1: image, param: param) { success, errer in
            if success!{
                self.hideBusy()
                self.getMessages()
            }
...
extension SendMessageSuperVC: UITableViewDataSource, UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if userID == messageObj.sender_id{
            if messageObj.media.isEmpty{
                ...
                cell.bubble.backgroundColor = .init(hex: "2e368e")
                return cell
            }
            else{
        ...
        DispatchQueue.global(qos: .background).async {
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    ManageAPI.shared.addMessageVideo(data: videoData, self.getFileExtension(from: videoURL), param: param) { success, errer in
                        if success!{
                            self.hideBusy()
                            self.getMessages()
                        }
...
extension SendMessageSuperVC: UITextFieldDelegate{
...
extension SendMessageSuperVC: UITextViewDelegate{
...
extension SendMessageSuperVC: UIImagePickerControllerDelegate, UINavigationControllerDelegate
{
...
extension SendMessageSuperVC: ImageViewerControllerDelegate {
