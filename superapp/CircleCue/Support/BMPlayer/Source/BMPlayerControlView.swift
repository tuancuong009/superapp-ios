...
@objc public protocol BMPlayerControlViewDelegate: AnyObject {
    ...
    func viewFeedCommentDetail()
    func viewFeedLikedDetail()
...
open class BMPlayerControlView: UIView {
    ...
    open weak var delegate: BMPlayerControlViewDelegate? {
        didSet {
            infoWrapperView.delegate = self.delegate
        }
    }
    ...
    /// bottom view
    open var infoWrapperView = DetailFeedView()
    ...
    // MARK: - Handle Info View
    
    func updateInfoViewUI(feed: NewFeed) {
        infoWrapperView.updateUI(newFeed: feed)
    }
    ...
    open func controlViewAnimation(isShow: Bool) {
        print("controlViewAnimation-->",isShow)
    ...
    func setupUIComponents() {
        ...
        backButton.isHidden = BMPlayerConf.hideBackButtonButton
        ...
        bottomMaskView.addSubview(infoWrapperView)
        ...
        fullscreenButton.isHidden = BMPlayerConf.hideFullscreenButton
...
//              make.top.equalToSuperview()
        ...
        infoWrapperView.snp.makeConstraints { [unowned self](make) in
            make.left.right.equalToSuperview()
            make.bottom.equalTo(self.bottomWrapperView.snp.top)
            make.top.equalToSuperview()
        }
        ...
        let fullscreenButtonWidth: CGFloat = BMPlayerConf.hideFullscreenButton ? 20.0 : 50.0
        fullscreenButton.snp.makeConstraints { [unowned self](make) in
            make.width.equalTo(fullscreenButtonWidth)
            make.height.equalTo(50)
            make.centerY.equalTo(self.currentTimeLabel)
