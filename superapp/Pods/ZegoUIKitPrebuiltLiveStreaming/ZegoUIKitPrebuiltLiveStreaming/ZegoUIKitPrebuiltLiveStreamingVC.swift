...
public class ZegoUIKitPrebuiltLiveStreamingVC: UIViewController {
    ...
    lazy var headIconView: ZegoLiveHostHeaderView = {
        ...
        iconView.btnProfile.addTarget(self, action: #selector(profileHostClick), for: .touchUpInside)
        return iconView
    }()
    lazy var roomLiveLabel: UILabel = {
        let label: UILabel = UILabel()
        label.numberOfLines = 0
        label.textAlignment = .center
        label.text = "Live"
    ...
    @objc public init(_ appID: UInt32, appSign: String, userID: String, userName: String, liveID: String, config: ZegoUIKitPrebuiltLiveStreamingConfig, _ avatar: String) {
        ...
        if config.role == .host {
            self.currentHost = ZegoUIKitUser.init(userID, userName, false, false, avatar)
        }
    ...
    func setupUI() {
        ...
        self.view.addSubview(self.roomLiveLabel)
    ...
    func setUIDisplayStatus() {
        if self.config.role == .host {
            ...
            if liveStatus == "1" {
                ...
                self.headIconView.isHidden = false
                self.roomLiveLabel.isHidden = false
                ...
                self.headIconView.isHidden = true
                self.roomLiveLabel.isHidden = true
    ...
    @objc func keyboardWillChangeFrame(node : Notification){
        ...
        UIView.animate(withDuration: duration) {
           ...
           // self.messageView.frame.origin.y = self.view.frame.size.height - 62 - bottomPadding - margin - self.inputViewHeight
        }
    }
    ...
    private func setupLayout() {
        ...
        self.roomLiveLabel.frame = CGRect(x: self.view.frame.size.width/2 - 52/2, y: self.memberButton.frame.minY, width: 52, height: 28)
        self.roomLiveLabel.layer.masksToBounds = true
        self.roomLiveLabel.layer.cornerRadius = 5
        
        self.bottomBar.frame = CGRect(x: 0, y: self.view.frame.size.height - 62 - bottomPadding, width: self.view.frame.size.width, height: 62)
        self.messageView.frame = CGRect(x: 16, y: self.view.frame.size.height - 62 - 300 - bottomPadding, width: UIkitLiveScreenWidth - 16 - 117, height: 300)
    ...
    @objc func profileHostClick(){
        if let host = self.currentHost{
            self.delegate?.viewProfileHost?(host)
        }
    }
